...
// Define interfaces for facility and booking details
interface IFacility {
  _id: string;
  name: string;
  description: string;
...
const Booking = () => {
  ...
  // Fetch facilities data using the API hook
  ...
  useEffect(() => {
    if (selectedFacility) {
      const facility = facilities?.data?.find(
        (f: IFacility) => f.name === selectedFacility
      );
      if (facility) {
        setFacilityDetails({
          name: facility.name,
          payableAmount: facility.pricePerHour.toString(),
        });
      }
    }
  }, [selectedFacility, facilities]);

  // Handle form input changes for booking details
  ...
  const calculationDuration = (startTime: string, endTime: string): number => {
    ...
    <div className="container mx-auto p-20">
      ...
      <div className="border-dotted border-2 p-4 mb-4">
        ...
        >
          ...
          {facilities?.data?.map((facility: IFacility) => (
